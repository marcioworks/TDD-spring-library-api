...
public class BookControllerTest {
    ...
    public void filterPageTest() throws Exception{
        Long id = 1L;
        ...
        String params = String .format("?title=%s&author=%s&page=0&size=100",book.getTitle(),book.getAuthor());

        MockHttpServletRequestBuilder request = MockMvcRequestBuilders
                .get(BOOK_API.concat(params))
                .accept(MediaType.APPLICATION_JSON);
        mvc .perform(request)
                .andExpect(status().isOk())
    ...
    private BookDTO createBook() {
        return BookDTO.builder().author("Marcio").title("as Aventuras").isbn("001").build();
    }
}
